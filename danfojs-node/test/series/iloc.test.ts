...
describe("Series iloc", function () {
    ...
    it("throw error for wrong row index value", function () {
        ...
        /* @ts-ignore */
        assert.throws(function () { df.iloc({ rows: 0 }) }, Error, `rows parameter must be an Array. For example: rows: [1,2] or rows: ["0:10"]`);
    });
    ...
    it("throw error for wrong string split parameter", function () {
        ...
        let df = new Series({ data });
        /* @ts-ignore */
        assert.throws(function () { df.iloc({ rows: ["0;1"] }); }, Error, `Invalid row split parameter: If using row split string, it must be of the form; rows: ["start:end"]`);
    });
    ...
    it("throw error for wrong string split value", function () {
        ...
        let df = new Series({ data });
        /* @ts-ignore */
        assert.throws(function () { df.iloc({ rows: ["0:a"] }); }, Error, `Invalid row split parameter. Split parameter must be a number`);
    });
    ...
    it("throw error for string split values greater than 2", function () {
        ...
        let df = new Series({ data });
        /* @ts-ignore */
        assert.throws(function () { df.iloc({ rows: ["0:4:2"] }); }, Error, `Invalid row split parameter: If using row split string, it must be of the form; rows: ["start:end"]`);
    });
    ...
    it("throw error for row index larger than series length", function () {
        ...
        let df = new Series({ data });
        assert.throws(function () { df.iloc({ rows: [0, 8] }); }, Error, "Invalid row parameter: row index 8 cannot be bigger than Series length 5");
    });

    it("throw error for non-numeric row index", function () {
        let data = [1, 2, 34, 5, 6];
        let df = new Series({ data });
        assert.throws(function () { df.iloc({ rows: [0, "4"] }); }, Error, "Invalid row parameter: row index 4 must be a number");
    });
    ...
    it(`throw error for wrong start index size ["0:20"]`, function () {
        ...
        let df = new Series({ data });
        /* @ts-ignore */
        assert.throws(function () { df.iloc({ rows: ["0:20"] }); }, Error, `row slice [end] index cannot be less than 5`);
    });
    ...
    it(`throw error for wrong start index size ["-1:2"]`, function () {
        ...
        let df = new Series({ data });
        /* @ts-ignore */
        assert.throws(function () { df.iloc({ rows: ["-1:2"] }); }, Error, `row slice [start] index cannot be less than 0`);
    });
