...
export default class Series extends NDframe implements SeriesInterface {
   ...
   */
  add(
    other: Series | Array<number> | number,
  ...
  add(
    ...
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("add");
  ...
  sub(
    ...
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("sub");
  ...
  mul(
    ...
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("mul");
  ...
  div(
    ...
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("div");
  ...
  pow(
    ...
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("pow");
  ...
  mod(
    ...
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("mod");
  ...
  private $checkAndCleanValues(
    ...
    if (this.dtypes[0] == "boolean") {
      values = utils.mapBooleansToIntegers(
        values as boolean[],
        1
      ) as ArrayType1D;
    }
   ...
   */
  mean(): number {
    const values = this.$checkAndCleanValues(
      this.values as ArrayType1D,
      "mean"
    );
   ...
   */
  count(): number {
    const values = utils.removeMissingValuesFromArray(
      this.values as ArrayType1D
    );
  ...
  maximum(other: Series | number | Array<number>): Series {
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("maximum");

    const newData = _genericMathOp({
      ndFrame: this,
      other,
      operation: "maximum",
    });
  ...
  sortValues(options?: {
    ...
    let sortedIdx = utils.sortArrayByIndex(
      rangeIdx,
      this.values,
      this.dtypes[0]
    );
  ...
  describe(): Series {
    if (this.dtypes[0] == "string") {
      throw new Error(
        "DType Error: Cannot generate descriptive statistics for Series with string dtype"
      );
    } else {
      const index = [
        "count",
  ...
  map(callable: mapParam, options?: { inplace?: boolean }): Series | void {
    ...
    const data = (this.values as ArrayType1D).map((val: any, i: number) => {
      if (isCallable) {
        ...
        throw new Error(
          "Param Error: callable must either be a function or an object"
        );
      }
    });
  ...
  abs(options?: { inplace?: boolean }): Series | void {
    ...
    if (this.dtypes[0] == "string")
      ErrorThrower.throwStringDtypeOperationError("abs");
    let newValues;
  ...
  private boolOps(
    ...
    if (typeof other == "number") {
      ...
      throw new Error(
        "ParamError: value for other not supported. It must be either a scalar, Array or Series"
      );
    }

    if (!(lSeries.length === rSeries.length)) {
      throw new Error(
        "LengthError: length of other must be equal to length of Series"
      );
    }
   ...
   */
  asType(
    dtype: "float32" | "int32" | "string" | "boolean" | "undefined",
    options?: { inplace?: boolean }
  ): Series;
  asType(
    dtype: "float32" | "int32" | "string" | "boolean" | "undefined",
  ...
  append(
    ...
    if (Array.isArray(newValue) && Array.isArray(index)) {
      if (newValue.length !== index.length) {
        throw Error(
          "Param Error: Length of new values and index must be the same"
        );
      }
  ...
  and(other: any): Series {
    ...
    if (other instanceof Series) {
      ...
      throw new Error(
        "Param Error: other must be a Series, Scalar, or Array of Scalars"
      );
    }
  ...
  iat(row: number): number | string | boolean | undefined {
    if (typeof row === "string") {
      throw new Error(
        "ParamError: row index must be an integer. Use .at to get a row by label."
      );
    }
  ...
  at(row: string): number | string | boolean | undefined {
    if (typeof row !== "string") {
      throw new Error(
        "ParamError: row index must be a string. Use .iat to get a row by index."
      );
    }
