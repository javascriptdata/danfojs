...
var StandardScaler = /** @class */ (function () {
    function StandardScaler() {
        this.$std = (0, tfjs_1.tensor1d)([]);
        this.$mean = (0, tfjs_1.tensor1d)([]);
    }
    StandardScaler.prototype.$getTensor = function (data) {
        var $tensorArray;
        if (data instanceof Array) {
            if (utils.is1DArray(data)) {
                $tensorArray = (0, tfjs_1.tensor1d)(data);
            }
            else {
                $tensorArray = (0, tfjs_1.tensor2d)(data);
            }
        }
        else if (data instanceof frame_1.default) {
            $tensorArray = (0, tfjs_1.tensor2d)(data.values);
        }
        else if (data instanceof series_1.default) {
            $tensorArray = (0, tfjs_1.tensor1d)(data.values);
        }
        else if (data instanceof tfjs_1.Tensor) {
            $tensorArray = data;
        }
        else {
    ...
    StandardScaler.prototype.fit = function (data) {
        ...
        this.$std = (0, tfjs_1.moments)(tensorArray, 0).variance.sqrt();
